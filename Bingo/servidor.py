...
class ServidorBingo:
    def __init__(
        ...
        print(
            f"Número mínimo de jogadores: {self.min_clientes}, máximo de jogadores: {self.max_clientes}"
        )
    ...
    def gerenciar_cliente(self, cliente_socket, endereco):
        ...
        try:
            ...
            with self.lock:
                ...
                if (
                    len(self.clientes_prontos) >= self.max_clientes
                    and not self.sorteio_iniciado
                ):
            ...
            while self.jogo_em_andamento:
                try:
                    ...
                    if not mensagem:  # Cliente desconectou
                        ...
                        break
                    if mensagem == "BINGO":
                        vencedor = self.nomes_jogadores.get(
                            cliente_socket, "Jogador desconhecido"
                        )
    ...
    def remover_cliente(self, cliente_socket):
        ...
        with self.lock:
            try:
                ...
                try:
                    cliente_socket.shutdown(
                        socket.SHUT_RDWR
                    )  # Adiciona shutdown antes do close
                    cliente_socket.close()
                except:
                    pass
                print(
                    f"Cliente removido. Jogadores restantes: {len(self.clientes_prontos)}"
                )
    ...
    def iniciar_temporizador(self):
        ...
        with self.lock:
            if not self.sorteio_iniciado:
                if (
                    len(self.clientes_prontos) >= 2
                ):  # Verifica se ainda tem pelo menos 2 jogadores
                    print(
